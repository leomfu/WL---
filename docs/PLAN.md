...
| 布局（2026-09-08 改版） | **顶部暗色导航条**（64px，fixed）+ 下面整幅宽度的浅色内容区。~~原为左侧 264px 暗色侧边栏 + 700px 内容列~~，站主要求改成常规网站那样的顶栏。唯一还限宽的是文章正文（`.prose-bw`，700px 阅读列） |
...
/hobbies         → 爱好（2026-09-08 合并）：页内三个筛选 —— 摄影 / 唱片 / 书影音。
                    摄影那栏开头是那张双色调海报（原在首页）；单辑详情仍在 /photos/[slug]
...
左边 Logo 圆徽 + 名字 + 八项平铺导航（首页/项目/视频/博客/爱好/工具/关于/联系，当前项下面一道细线），
右边 中/EN 切换 · 分隔线 · 社交图标（X/GitHub/哔哩哔哩/YouTube/小红书/抖音）· ⌘K。
颜色仍是原来那套 shell-* 暗侧灰阶 —— 那块暗色只是从一竖条变成了一横条。
**断点**：`<lg` 汉堡 + 全屏抽屉；`lg` 导航平铺、社交先收起；`xl` 社交图标一起出来。
合并掉的旧板块（摄影/唱片/书影音/新闻/小工具）仍能从 ⌘K 搜到，跳到合并后页面的对应筛选（`/hobbies#photos` 这种）。
